...
The v0.1 scorecard is intentionally limited. It proves the runtime is controlling and recording actions, not that the actions created business value. It is available via `agentguard scorecard --agent examples/agent.yaml --receipts .agentguard/receipts.jsonl` and supports both JSONL and SQLite stores. See `docs/example-report.md` for concrete output.
